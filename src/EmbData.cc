...
void EmbData::addBits (BitString addbits)
{
	...
	switch (State) {
		...
		case READ_VERSION:
		{
			std::cout << "read version" << std::endl;
#ifdef DEBUG
		...
		case READ_ENCINFO: {
			std::cout << "read encinfo" << std::endl;
#ifdef DEBUG
		...
		case READ_NPLAINBITS: {
			std::cout << "read plainbits" << std::endl;
#ifdef DEBUG
		...
		case READ_ENCRYPTED: {
			std::cout << "read encrypted" << std::endl;
#ifdef DEBUG
...
BitString EmbData::getBitString ()
{
	...
	if (Compression > 0) {
		...
		std::cout << compr.getLength() << std::endl;
		std::cout << Compression << std::endl;
